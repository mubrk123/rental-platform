...
router.post("/vehicle-images/:vehicleId", adminOnly, upload.array("images", 10), async (req, res, next) => {
  try {
    ...
    const urls = files.map(f => f.path); // store path; frontend will request via static serving
...
/**
 * Upload user documents (if not doing via /api/bookings)
 * fieldname: 'documents' (multiple)
 */
