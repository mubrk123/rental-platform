...
router.get("/pdf/booking/:id", async (req, res) => {
  try {
    ...
    const locData = await Location.findOne({ name: { $regex: new RegExp(`^${booking.city}$`, "i") } }).lean();
    const locationCode = locData?.locationCode || booking.city?.slice(0, 3).toUpperCase();
